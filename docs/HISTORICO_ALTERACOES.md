...
## 2026-09-25 - Leilão: "mesa show" do locutor, em teste ao lado da clássica

### Resumo
Pedido do clube: uma segunda mesa do locutor, só para teste, totalmente reformulada — dinâmica,
bonita, com todas as informações do leilão à mão. Abre em `/locutor/<id>/nova/`; a clássica
continua a padrão, e as duas se apontam por um link no topo.

### Como foi feita
O mesmo molde da tela show do público: **um motor só**. O `locutor.js` continua fazendo tudo (abrir
item, VENDIDO com a confirmação, a pergunta de abrir outro com item em disputa, microfone, mudo, chat,
pagamentos e sons) e passou a **emitir avisos** `mesa:*` em `try/catch`. A mesa nova é outro template,
com **todos os ids que o motor procura**, e o `mesa_show.js` só escuta e desenha — sem requisição
nenhuma.

### O que a mesa show tem
- **Placar da noite** no topo: vendido (e recebido), itens batidos com barra de progresso, fila (e
  quantos ficaram sem lance), maior arremate, lances na noite (e quantas pessoas deram lance), média
  por item. Os números rolam quando mudam.
- **O item em pregão**: foto grande sem corte, valor atual enorme, quem está ganhando (acende a cada
  lance, como na clássica), o **anel do silêncio** (enche de 0 a 30 s sem lance, âmbar em 15 s e
  vermelho em 30 s) e o **ritmo da disputa** (lances no último minuto; o fundo da tela esquenta junto).
- **Lances**: gráfico do valor lance a lance, **quem está na disputa** (lances e maior valor de cada
  um — a narração pronta para o locutor) e a linha do tempo. O chat ao lado.
- **A seguir**: o próximo item com foto, medidas e lance inicial; a fila com o botão de abrir.
- Embaixo, sala online, voz e pagamentos (três cards, três colunas).
- Aba nova **🏆 Noite**: andamento (batidos / em pregão / fila / sem lance), quem mais arrematou e os
  últimos batidos, com pago ou a pagar. A aba Itens virou vitrine com foto.
- Carimbo **NOVO ITEM / VENDIDO! / SEM LANCE** com confete, **dentro da foto**.
- O estado da **voz** no topo o tempo todo (desligada / NO AR / NO MUDO / caiu) e a hora.

### Arquivos criados/alterados
- `templates/leilao/locutor_show.html`, `static/leilao/css/mesa_show.css`, `static/leilao/js/mesa_show.js`
  (novos).
- `leilao/views.py`: `locutor_show_view` e o miolo `_mesa`, compartilhado com a clássica;
  `_resumo_da_noite`; `/locutor/dados/` aceita `?resumo=1`.
- `leilao/urls.py`: `/locutor/nova/` e `/locutor/<id>/nova/`.
- `static/leilao/js/locutor.js`: `emitir` + os avisos; pede `resumo=1` quando o template diz
  (`data-resumo`); a nota da sala dizia "conexãoões".
- `templates/leilao/locutor.html`: link "✨ Mesa show".
- `leilao/tests.py`: `MesaShowTests` (25 testes).

### Decisões tomadas
- **O placar só vem com `?resumo=1`**: a mesa padrão continua recebendo exatamente o que recebia, sem
  consulta a mais. É dinheiro e nome — GET autenticado, nunca broadcast (há teste).
- **Placar empilhado dentro do card do item**: lado a lado com o nome, "R$ 75,00" quebrava em duas
  linhas — e é o número que o locutor lê em voz alta.
- **No celular a foto fica mais baixa** (16:10, no máximo 34% da altura): quadrada, ela empurrava o
  valor e o VENDIDO para baixo da dobra.
- **O número que rola usa só o relógio do `requestAnimationFrame`** e garante o valor final por
  `setTimeout` — a regra que a tela show do público aprendeu com o "R$ -6,17".

### Verificação
- `MesaShowTests` e a suíte inteira do leilão.
- Conferida no Chrome headless sobre um banco descartável com dados fictícios, em 1440, 1100 e 420 px:
